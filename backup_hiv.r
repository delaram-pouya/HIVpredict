...
### unable to perform parameter learning from partial data 
### 
...
bn.cpts  <- ReadProbStruct("HIV_CPTs2.txt")
# dynamic part
dyn.str  <- read.table( "HIV_Dyn_Struct.txt", sep=",", header=TRUE)
dyn.cims <- ReadProbStruct("HIV_CIMs2.txt")
...
print("bn: d.NRTI.1$`VL=1`")
print( cptsOut$d.NRTI.1$`VL=1`)
print(cptsOut3$d.NRTI.1$`VL=1`)
print(sqrt(sum((cptsOut$d.NRTI.1$`VL=1`-cptsOut3$d.NRTI.1$`VL=1`)^2)))

print("dynamics: d.NRTI.1$`VL=1,g.NRTI.1=2`")
print(cimsOut$d.NRTI.1$`VL=1,g.NRTI.1=1`)
print(cimsOut3$d.NRTI.1$`VL=1,g.NRTI.1=1`)
print(sqrt(sum((cimsOut$d.NRTI.1$`VL=1,g.NRTI.1=1`-cimsOut3$d.NRTI.1$`VL=1,g.NRTI.1=1`)^2)))
